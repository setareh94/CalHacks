    ...
    if (cell == nil) {
        ...
        if ([indexPath section] == 0) {
            ...
            if ([indexPath row] == 0) {
                ...
                takePictureButton.frame = CGRectMake(60, 5, 90, 30);
                ...
                UIImage *noPictureImage = [UIImage imageNamed:@"IconNoPicture"];
                
                UIGraphicsBeginImageContext(self.imagePreviewView.frame.size);
                [noPictureImage drawInRect:self.imagePreviewView.bounds];
                UIImage *image = UIGraphicsGetImageFromCurrentImageContext();
                UIGraphicsEndImageContext();
                
                self.imagePreviewView.backgroundColor = [UIColor colorWithPatternImage:image];
                
                
                UIButton *useDropBoxButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
                [useDropBoxButton addTarget:self
                                      action:@selector(useDropBox)
                            forControlEvents:UIControlEventTouchUpInside];
                [useDropBoxButton setTitle:@"Use DropBox" forState:UIControlStateNormal];
                useDropBoxButton.frame = CGRectMake(160, 5, 110, 30);
                
                [cell addSubview:self.imagePreviewView];
                [cell addSubview:takePictureButton];
                [cell addSubview:useDropBoxButton];
            }else {
